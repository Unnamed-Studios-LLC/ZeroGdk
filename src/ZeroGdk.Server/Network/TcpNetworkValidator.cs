...
namespace ZeroGdk.Server.Network
{
	internal sealed class TcpNetworkValidator(NetworkKeyStore networkKeyStore,
		...
		IOptions<ClientOptions> clientOptions) : IExpire
	{
		...
		private void OnReceivedKey(string key, PendingSocket pendingSocket)
		{
			...
			var networkClient = new TcpNetworkClient(pendingSocket.Socket, _clientOptions.MaxReceiveQueueSize, _clientOptions.ReceiveBufferSize);
			var client = new Client(networkClient, _serviceProvider, request.Route, request.ClientId, _encoding, _clientOptions);
			_clientQueue.Create(request, client);
		}
